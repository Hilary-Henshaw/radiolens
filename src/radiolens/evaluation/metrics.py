...
def _compute_roc_auc(y_true: np.ndarray, y_score: np.ndarray) -> float:
    ...
    return float(np.trapz(tprs_arr[sorted_idx], fprs_arr[sorted_idx]))  # type: ignore[attr-defined]
...
def _compute_pr_auc(y_true: np.ndarray, y_score: np.ndarray) -> float:
    ...
    return float(np.trapz(precisions_arr[sorted_idx], recalls_arr[sorted_idx]))  # type: ignore[attr-defined]
